...
class BigCombination(object):
    ...
    def __init__(self, mod: int=10**9+7, max_n: int=10**6+1):
        fac, inv=[1], []
...
def main():
    ...
    eight_pow = [ModInt(1)]
    for i in range(N):
        nice_pow.append(nice_pow[-1]*9)
        eight_pow.append(eight_pow[-1]*8)

    for i in range(1,N):
        ans += B.get_combination(N,i) * (nice_pow[N-i] - eight_pow[N-i])
