...
//左側いけるところまで伸ばす
void extend_left_to_lim(int i,Rect& LRTB, int X, int Y, vector<pair<int,int>> const& LT, vector<pair<int,int>> const& WH){
    int left_l = -1, left_r = X;
    while(abs(left_r - left_l) > 1){
        int mid = (left_r + left_l) / 2;
        get<0>(LRTB) = mid;
...
//上側いけるところまで伸ばす
void extend_top_to_lim(int i,Rect& LRTB, int X, int Y, vector<pair<int,int>> const& LT, vector<pair<int,int>> const& WH){
    int top_u = -1, top_d = Y;
    while(abs(top_d - top_u) > 1){
        int mid = (top_d + top_u) / 2;
        get<2>(LRTB) = mid;
...
//右側いけるところまで伸ばす
void extend_right_to_lim(int i,Rect& LRTB, int X, int Y, vector<pair<int,int>> const& LT, vector<pair<int,int>> const& WH){
    int right_l = X+1, right_r = WIDTH+1;
    while(abs(right_r - right_l) > 1){
        int mid = (right_r + right_l) / 2;
        get<1>(LRTB) = mid;
...
//下側いけるところまで伸ばす
void extend_bottom_to_lim(int i,Rect& LRTB, int X, int Y, vector<pair<int,int>> const& LT, vector<pair<int,int>> const& WH){
    int bottom_u = Y+1, bottom_d = HEIGHT+1;
    while(abs(bottom_d - bottom_u) > 1){
        int mid = (bottom_d + bottom_u) / 2;
        get<3>(LRTB) = mid;
    ...
    for(auto i:indices1){
        ...
        if(WH[i]==mp(0,0) or not scored[i]){ //まだ置いてない or 指定地点に置かれてない広告をどうにかする
            ...
            if(check(X, Y, LT, WH)){ //広告の場所におけるなら置く
                ...
                for(auto j:indices2){ 
                    if(j==0){
                        extend_left_to_lim(i,LRTB,X,Y,LT,WH);
                    }else if(j==1){
                        extend_right_to_lim(i,LRTB,X,Y,LT,WH);
                    }else if(j==2){
                        extend_top_to_lim(i,LRTB,X,Y,LT,WH);
                    }else if(j==3){
                        extend_bottom_to_lim(i,LRTB,X,Y,LT,WH);
                    }
...
long double get_score(vector<Rect>& XYRi, vector<Rect>& ans){
    ...
    REP(i,N){
        ...
        if(l<=X and X<r and t<=Y and Y<b) {
            ...
            // debug((1-(1-(long double)min(R,w*h)/max(R,w*h))*(1-(long double)min(R,w*h)/max(R,w*h))))
        }else{
            // debug(i)
            // debug(mp(X,Y))
            // debug(mp(l,r))
...
void random_change(vector<Rect>& XYRi, vector<pair<int,int>>& LT, vector<pair<int,int>>& WH, vector<bool>& scored, vector<Rect>& ans, clock_t end_time){
    ...
    while(true){
        ...
        REP(_,1000){
            ...
            vector<int> indices1; REP(i,N) indices1.emplace_back(i); shuffle(ALL(indices1), engine); 
...
int main(){
    ...
    // auto score = get_score(XYRi, ans);
    // debug(int(score))

    random_change(XYRi, LT, WH, scored, ans, end_time);  //これの最後でansを過去ベストに更新してるのでLT, WHはこの時点でansとは一致する保証はない.

    

    // score = get_score(XYRi, ans);
    // debug(int(score))
