...
void set_step1(vector<Rect>& XYRi, vector<pair<int,int>>& LT, vector<pair<int,int>>& WH, vector<bool>& scored){
    ...
    REP(i,N){
        ...
        if(last_x<=x){
            ...
            for(auto left:x_list){
                for(auto top:y_list){
                    if(top > y) continue;
                    for(auto bottom:y_list){
                        if(top>=bottom) continue;
                        if(bottom < y) continue;

                        // debug(mp(top, bottom))
                        int right = left + (r + (bottom - top - 1)) / (bottom-top);
                        // debug(right)

                        Rect LRTB = make_tuple(left, right, top, bottom);
                        if( LT[i].first + WH[i].first > right and check_board(i, LRTB, LT, WH) ){
                            // if( abs(r-(right-left)*(bottom-top)) > abs(r-(right+1-left)*(bottom-top)) ){
                            //     right++;
                            // }
                            // else if( abs(r-(right-left)*(bottom-top)) > abs(r-(right+1-left)*(bottom-1-top)) ){
                            //     right++; bottom--;
                            // }
            ...
            last_x = left + width;
            x_list.emplace_back(last_x);
            y_list.insert(top);
            y_list.insert(top + height);
            // debug(x_list)
            // debug(y_list)
            scored[i] = true;
        }else{
