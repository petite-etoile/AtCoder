...
bool partial_swap_round(vector<vector<int>>& league_table, int team1, int slot1, int slot2, vector<vector<int>> const& dist_matrix, int& val, bool flag){
    int cost_diff = 0;

    vector<int> teams = get_teams_changed_partial_swap_round(league_table, team1, slot1, slot2);
    // debug(teams)

    if(flag) cost_diff -= get_partial_cost_partial_swap_round(league_table, teams, slot1, slot2, dist_matrix); 
