...
int main(){
    ...
    clock_t start = clock();
    clock_t stop = start + 39 * CLOCKS_PER_SEC / 20;
    ...
    vector<int> best_sol = T;
    int64 best_val = score;
    ...
    while(true){
        ...
        REP(_,100){
            ...
            if(chmax(best_val, new_val)) best_sol = T;

            prob = exp( (new_val - score) / temp);
            if(prob > xor128() % rand_max / (double)rand_max){
                //解を更新
                score = new_val;
            }else{
                //解を更新しないので戻す
