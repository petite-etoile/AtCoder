...
//step4(改善)
void set_step4(vector<Rect>& XYRi, vector<pair<int,int>>& LT, vector<pair<int,int>>& WH, vector<bool>& scored, vector<Rect>& ans, vector<int> const& indices){
    int N = XYRi.size();
    ...
    for(auto i:indices){
        ...
        if(WH[i]==mp(0,0) or not scored[i]){ //置く場所なかった広告は適当にあれする(とりあえず)
            ...
            if(check(X, Y, LT, WH)){
                ...
                WH[i] = make_pair(width, height);
                scored[i] = true;
                ans[idx] = make_tuple(left,top,left+width,top+height);
            }
        }
...
void random_change(vector<Rect>& XYRi, vector<pair<int,int>>& LT, vector<pair<int,int>>& WH, vector<bool>& scored, vector<Rect>& ans, clock_t end_time){
    ...
    while(true){
        ...
        REP(_,1000){
            ...
            vector<int> indices; REP(i,N) indices.emplace_back(i);
            shuffle(ALL(indices), engine);

            remove_ad(xor128() % N, WH, scored);

            set_step4(XYRi,LT,WH,scored,ans,indices);
