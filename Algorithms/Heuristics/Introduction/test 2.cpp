...
int64 replace_contest(int D, vector<int64> const& C, vector<vector<int64>> const& S, vector<int>& T, int64 pre_val, int date, int q, vector<set<int>>& contest_schedule, int& p){
...
int main(){
    ...
    while(true){
        ...
        double temp = get_temp(now, start, stop, start_temp, end_temp);
        REP(_,5){
            REP(date, D){
                REP(contest, 26){
                    int ori;
                    int64 new_val = replace_contest(D, C, S, T, score, date, contest, contest_schedule, ori);
                    assert(new_val == calc_score(D,C,S,T));
                    double prob = exp( (new_val - score) / temp);
