...
int main(){
    ...
    vector<vector<int64>> AB(N);
    vector<int64> send_num(N);
    REP(i,N){
    ...
    StronglyConnectedComponents SCC(edge); //横着。
    vector<vector<int64>> cycles(SCC.get_scc_size());
    vector<bool> visited(N,false);
    REP(i,N){
        if(visited[i]) continue;
        int idx = SCC.get_topological_index(i);
        int now = i;
        while(not visited[now] and SCC.get_topological_index(now) == idx){
            cycles[idx].emplace_back(now);
            visited[now] = true;
            now = edge[now][0];
        }
    }
    ...
    int64 ans=0;
    REP(idx,SCC.get_scc_size()){
        auto& cycle = cycles[idx];
        if(cycle.size() == 1){
            int from = cycle[0];
            int to = edge[from][0]; //この人がプレゼントを渡す相手
            ans += send_num[from] * max(AB[to][0], AB[to][1]); //喜ぶ方を後出しで渡すことが出来る
        }else{
            cycle.emplace_back(cycle[0]);
            vector<vector<int64>> DP(cycle.size(),{0,0}); //DP[i][j]:=(サイクルの中で)i番目から、j(\in {0,1})を渡すときの(iまでの)喜び合計最大値 (i=0のとき、j=0とする)
            for(int i=0; i<cycle.size()-1; i++){
                int from = cycle[i], to = cycle[i+1];
                REP(j,2){
                    if(i==0 and j==1) continue; 
                    chmax(DP[i+1][j  ], DP[i][j] + send_num[from] * AB[to][0]); //同じものをあげる
                    chmax(DP[i+1][j^1], DP[i][j] + send_num[from] * AB[to][1]); //違うものをあげる
                }
            }
            ans += DP.back()[0];
        }
